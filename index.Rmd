...
s1 <- ggplot(data=singleyr_lka2,
       aes(y=reorder(authfull,(nc2222_ns)), x = nc2222_ns, col=inst_name)) + geom_point()   + xlab("Total cites in 2022") + scale_color_manual(values = color_ins) + theme(legend.position="none") + ylab("")
...
### Total cites in 2022 (self-citations excluded)

```{r, fig.height=8}
ggplotly(s1, tooltip = c("inst_name","nc2222_ns"))
```

### hm-index as of end 2022 (self-citations excluded)
...
### h-index as of end 2022 (self-citations excluded)
...
### Composite score (self-citations excluded)
...
### Total cites to single authored papers
...
```{r, fig.height=8}
career_lka2 <- career_lka
career_lka2 <- clean_names(career_lka2)
...
### Total cites 1996-2022 (self-citations excluded)
