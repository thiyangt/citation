...
```{r, fig.height=12}
a1 <- ggplot(data=career_lka_filter,
       aes(y=reorder(authfull,(-rank)), x = rank, col=inst_name)) + geom_point() +   geom_segment(aes(yend = reorder(authfull,(-rank))), xend = 0, colour = "grey50") +
  ylab("Author name") + xlab("Rank")  + scale_x_continuous(labels = scales::comma_format()) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

a2 <- ggplotly(a1, tooltip = c("rank"))
a2 |> style(showlegend = FALSE)
```
