...
s1 <- ggplot(data=singleyr_lka2,
       aes(y=reorder(authfull,(nc2222_ns)), x = nc2222_ns), col=inst_name) + geom_point()   + xlab("Total cites in 2022 (self-citations excluded)") + scale_color_manual(values = color_ins) + theme(legend.position="none") 

s2 <- ggplot(data=singleyr_lka2,
       aes(y=reorder(authfull,(h22_ns)), x = h22_ns), col=inst_name) + geom_point() +    xlab("h-index as of end 2022 (self-citations excluded)") + scale_color_manual(values = color_ins) + theme(legend.position="none") 
ggplotly(s1, tooltip = c("inst_name","h22_ns"))
```
...
s3 <- ggplot(data=singleyr_lka2,
       aes(y=reorder(authfull,(c_ns)), x = c_ns, col=inst_name)) + geom_point() +    xlab("Composite score (self-citations excluded)") + scale_color_manual(values = color_ins)+theme(legend.position="none") 
ggplotly(s3, tooltip = c("inst_name","nc2222_ns"))
```
